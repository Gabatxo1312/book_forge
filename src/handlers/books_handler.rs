...
async fn show_book(
    ...
    let current_holder = match book.clone().current_holder_id {
        Some(b) => user::Entity::find_by_id(b)
            .one(&state.db)
            .await
            .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?,
        None => None,
...
async fn new_book(State(state): State<Arc<AppState>>) -> Result<Html<String>, StatusCode> {
    let users = user::Entity::find()
        .all(&state.db)
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;

    Ok(Html(NewBookTemplate { users }.render().unwrap()))
}
...
async fn edit_book(
    ...
    match book {
        Some(b) => Ok(Html(EditBookTemplate { book: b, users }.render().unwrap())),
...
async fn update_book(
    ...
    let book_by_id = book::Entity::find_by_id(id)
        .one(&state.db)
        .await
        .map_err(|_| StatusCode::NOT_FOUND)?;
...
async fn search_book_in_open_library(
    ...
    match query_value {
        Some(val) => match services::open_library::get_book_from_api(Some(val)).await {
            Ok(books) => Ok(Json(books)),
            Err(e) => {
                Err(services::errors::ApiError::OpenLibraryError(e.to_string()).into_response())
            }
        },
